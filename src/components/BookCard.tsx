...
export default function BookCard({ book, priority = false }: { book: BookDoc; priority?: boolean }) {
  ...
  return (
    <Link href={href} className="book-card block group">
      <div className="relative aspect-[2/3] bg-cream-dark rounded-md overflow-hidden mb-2">
        {book.cover_i ? (
          <Image
            src={cover}
            alt={book.title}
          ...
          <div className="w-full h-full flex items-center justify-center bg-cream-dark text-muted text-xs text-center px-3">
            {book.title}
          </div>
        )}
      </div>
