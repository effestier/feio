 ...
 */
export async function getByGenre(genreSlug: string, limit = 20, offset = 0, coversOnly = false): Promise<{ books: BookDoc[]; hasMore: boolean }> {
  ...
  const perKeyword = Math.max(Math.ceil((offset + limit) * 3 / keywords.length) + 5, 20);
