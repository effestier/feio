...
export async function GET(
  ...
  // Find the best readable format: HTML > DjVu > Plain Text
  const files = meta.files || [];
  const htmlFile = files.find(
    (f) => f.format === "HTML" && !f.name.includes("_cover") && !f.name.includes("_toc")
  );
  const djvuFile = files.find((f) => f.format === "DjVu");
  const txtFile = files.find(
    (f) => (f.format === "Plain Text" || f.format === "DjVuTXT") && !f.name.includes("_cover")
  );
  ...
  if (htmlFile) {
    contentUrl = `https://archive.org/download/${id}/${htmlFile.name}`;
  } else if (djvuFile) {
    // DjVu can't be read as text — use the IA detail page embed
    contentUrl = null;
  } else if (txtFile) {
    contentUrl = `https://archive.org/download/${id}/${txtFile.name}`;
    isText = true;
  }

  if (!contentUrl) {
    return NextResponse.json(
      { error: "No readable text format available" },
      { status: 404 }
    );
  }
  ...
  try {
    ...
    if (isText) {
      ...
      // HTML → extract body, strip IA chrome
      const bodyMatch = raw.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
      cleaned = bodyMatch ? bodyMatch[1] : raw;
      // Remove IA navigation/chrome
      cleaned = cleaned
        .replace(/<div[^>]*class="[^"]*ia-[^^"]*"[^>]*>[\s\S]*?<\/div>/gi, "")
        .replace(/<div[^>]*id="[^"]*nav[^"]*"[^>]*>[\s\S]*?<\/div>/gi, "")
        .replace(/<script[\s\S]*?<\/script>/gi, "")
        .replace(/<style[\s\S]*?<\/style>/gi, "");
    }

    const wrapped = wrapContent(cleaned, title, id, author);
...
<body>
  <h1>${title}</h1>
  ${author !== "Unknown" ? `<p style="text-align:center;color:#666;font-size:0.9em;margin-bottom:2em;">${author}</p>` : ""}
  ${content}
  <div class="ia-attr">
  ...
  <script>
    (function() {
      ...
      var SCROLL_KEY = 'feio-scroll-ia-${identifier}';
